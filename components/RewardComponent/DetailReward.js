...
const DetailsReward = ({ navigation, route }) => {
    ...
    return (
        <SafeAreaView style={styles.containers}>
            <ImageBackground style={styles.headerImage} source={{ uri: voucher.image }}>
            ...
            <ScrollView
                ...
                scrollEnabled={true}
                bounces={true}>
                    <Text style={styles.rw_title}>{voucher.title}</Text>
                    <ListItem> Mã giảm giá
                        <Text style={styles.rw_saleoff}> {voucher.code}</Text>
                    </ListItem>
                    <ListItem> Chúc mừng bạn đã nhận được
                        <Text style={styles.rw_saleoff}> Coupon {voucher.sale_off}</Text>
                    </ListItem>
                    <ListItem> Thời gian áp dụng:
                        <Text style={styles.rw_saleoff}> {voucher.date_start} - {voucher.date_end}</Text>
                    </ListItem>
